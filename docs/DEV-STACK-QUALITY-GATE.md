...
- `HERMES_HOME` stays under `${OPENATLAS_HOME:-$HOME/.openatlas}`.
- The live local `$HOME/.hermes` service is never killed by name.
...
```bash
backend/.venv/bin/python backend/scripts/quality_gate.py
```
