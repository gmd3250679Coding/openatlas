...
case "$OPENATLAS_HOME" in
  /*) ;;
  *) echo "[start.sh][FATAL] OPENATLAS_HOME must be absolute: $OPENATLAS_HOME" >&2; exit 1 ;;
esac
...
# 2. We refuse if any path under the live ~/.hermes appears in the
...
if [[ -r "/proc/$HERMES_PID/environ" ]]; then
  ...
  # 1c) Must NOT contain the live ~/.hermes path anywhere
